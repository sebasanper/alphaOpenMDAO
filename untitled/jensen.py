    ...
    for i in range(0, np.size(x)):
        ...
        if dx > 0:
            ...
            if d == 0:
                ...
                overlap_area = rdown ** 2. * sp.arccos(
                    (d ** 2. + rdown ** 2. - R ** 2.) / (2.0 * d * rdown)) + R ** 2. * sp.arccos(
                    (d ** 2. + R ** 2. - rdown ** 2.) / (2.0 * d * R)) - 0.5 * sp.sqrt(
                    (-d + rdown + R) * (d + rdown - R) * (d - rdown + R) * (d + rdown + R))
...
def loss(r_0, a, alpha, x_focus, x, y_focus, y, overlap):
    ...
    for i in range(0, np.size(x)):
        ...
        R = r_0[i] + dx * alpha
        if dx > 0:
            loss[i] = overlap[i] * 2. * a * (r_0[i] / (r_0[i] + alpha * (dx))) ** 2 * 0.5 * (
            np.cos(-dy * pi / (R + 4 * r_0[i])))
